function [XYZ_smooth, D_smooth, tangent_vec, n_seg] = BsplineSmoothEqualSeg(XYZ, D, mode, n_sample, len_seg)
    ...
    % - len_seg: the largest length of the short branch
    ...
    % - n_seg: the number of short branches
    ...
    if nargin < 4
        ...
        % n_seg = 7; % Can be set as input parameter
        len_seg = 14;
    end
    ...
    if (l_spline(end) > len_seg)
        n_seg = round(l_spline(end) / len_seg);
    ...
    if (mode == 1)% branch -> branch
        seg_desire = linspace(D(1), (l_spline(end) - D(end)), n_seg + 1);
        seg_desire = [0, seg_desire, l_spline(end)];
    elseif mode == 2% branch -> terminal
        seg_desire = linspace(D(1), l_spline(end), n_seg + 1);
        seg_desire = [0, seg_desire];
    elseif mode == 3% root -> branch
        seg_desire = linspace(0, (l_spline(end) - D(end)), n_seg + 1);
