...
% %%
%% User settings
% trees{1}=load_tree('job_checked_0001.swc');
% xplore_tree;
%%
num_geo = 1105
for idx_geo = 1:num_geo
    ...
    input_file=[skeleton_path,'job_checked_', num2str(idx_geo,'%04d'),'.swc'];
    smooth_file=[io_path,num2str(idx_geo,'%04d'),'//skeleton_smooth.swc'];
    % tangent_file=[io_path,neuron_name,'_tangent.txt'];
    
    %     parameter_file = strjoin(parameter_file,'');
    %     input_file = strjoin(input_file,'');
    %     smooth_file = strjoin(smooth_file,'');
