...
% skeleton_raw_path = '..//..//MLdata//data_neuron_skeleton//raw//';
% skeleton_smooth_path = '..//..//MLdata//data_neuron_skeleton//smooth//';
% pipe_path = '..//..//MLdata//skeleton_pipe//';

% parameter_file = [io_path, 'mesh_parameter.txt'];
% skeleton_input = [io_path, num2str(idx_geo, '%04d'), '//skeleton_smooth.swc'];
...
for idx_geo = 1:num_geo
    start_trees;
    clearvars -except idx_geo num_pipe
    % idx_geo

    skeleton_path = '..//..//MLdata//data_neuron_skeleton//';
    skeleton_raw_path = '..//..//MLdata//data_neuron_skeleton//raw//';
    skeleton_smooth_path = '..//..//MLdata//data_neuron_skeleton//smooth//';
    pipe_path = '..//..//MLdata//skeleton_pipe//';

    parameter_file = [skeleton_path, 'mesh_parameter//', 'geo_', num2str(idx_geo), '.txt'];
    input_file = [skeleton_raw_path, 'geo_', num2str(idx_geo), '.swc'];
    smooth_file = [skeleton_smooth_path, 'geo_', num2str(idx_geo), '_smooth.swc'];
    output_file = [pipe_path, 'pipe_', num2str(num_pipe), '.swc'];
    % input_file = [skeleton_raw_path, 'geo_', num2str(idx_geo, '%04d'), '.swc'];
    % smooth_file = [skeleton_smooth_path, 'geo_', num2str(idx_geo, '%04d'), '_smooth.swc'];
    % output_file = [pipe_path, 'pipe_', num2str(num_pipe, '%04d'), '.swc'];
    ...
    for i = 1:n_sect
        ...
        for idx_chop = 1:tmp_n_chop
            ...
            % output_pipe_mat = [output_pipe_mat; [j + 1, 2, tmp_XYZ(idx_chop* n_seg_per_chop , :), d_after(sect_after(i, 2)) / 2, j]];
