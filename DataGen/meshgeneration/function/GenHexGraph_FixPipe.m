function [] = GenHexGraph(io_path)
    ...
    for index_bif = 1:n_bif
        ...
        [idx_bif, ~] = find(dict_bif(:, 2) == i);
        [idx_pipe, ~] = find(dict_pipe(:, end) == par_node);
        ...
        lidx1_extract = [1:m1_extract] + m1_extract * (col - 1);
        ...
        [idx_bif, ~] = find(dict_bif(:, 2) == i);
        [idx_pipe, ~] = find(dict_pipe(:, 2) == bifur1_node);
        ...
        [idx_bif, ~] = find(dict_bif(:, 2) == i);
        [idx_pipe, ~] = find(dict_pipe(:, 2) == bifur2_node);
    ...
    for index_sec = 1:n_sect
        ...
        if termination(index_end_pt)
            [idx_pipe, ~] = find(dict_pipe(:, end) == index_end_pt);
        ...
        for index_sec_pt = 2:sect_ptnum - 1
            ...
            [idx_pipe, ~] = find(dict_pipe(:, 2:end) == idx_node);
        ...
        for index_sec = 1:n_sect
            ...
            if (index_start_pt == 1)
                ...
                for index_sec_pt = sect_ptnum:-1:2
                    ...
                    if num_pipe == 1
                        lidx1_extract = [1:m1_extract] + m1_extract * (col - 1);
                        lidx2_extract = [1:m1_extract] + m1_extract * (col - 1);
                    else
                        lidx1_extract = [1:m1_extract] + m1_extract * (col(2) - 1);
                        lidx2_extract = [1:m1_extract] + m1_extract * (col(1) - 1);
                    end
            ...
            if (termination(index_end_pt))
                ...
                for index_sec_pt = 2:sect_ptnum
                    ...
                    [idx_pipe, col] = find(dict_pipe(:, 2:end) == i);
                    num_pipe = size(idx_pipe, 1);
                    gidx_extract = [1:m1_extract] + tmp_pointnumber_extract;
    ...
    for idx_sim = 1:n_pipe_sim
        ...
        dlmwrite(fname_sim_edge_txt, edge, 'Delimiter', '\t');
        ...
        dlmwrite(fname_sim_C2Smapping, C2Smapping, 'Delimiter', '\t');

        % Output the mapping (Extracted simple mesh to the simulator)
        fname_sim_S2Dmapping = [sim_path, 'simulator_', num2str(idx_global_sim), '_S2Dmapping.txt'];
        dlmwrite(fname_sim_S2Dmapping, S2Dmapping - 1, 'Delimiter', ',');
    end
    ...
    for idx_sim = 1:n_bif_sim
        ...
        C2Smapping(:, 1) = C2Smapping(:, 1) - 1;
        dlmwrite(fname_sim_mapping, C2Smapping, 'Delimiter', '\t');
    ...
    % dlmwrite(mapping_extract_output,PointMapping_extract'-1, 'Delimiter','\n');
    dlmwrite(mapping_extract_output, PointMapping_extract' - 1, 'Delimiter', ',');
