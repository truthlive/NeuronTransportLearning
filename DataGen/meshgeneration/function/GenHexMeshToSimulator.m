function [] = GenHexMeshToSimulator(io_path, pipe_start, bif_start)
    %% User settings
    ...
    for idx_sim = 1:n_pipe_sim
        ...
        fname_sim = [fld_sim, 'simulator.vtk'];
        ...
        fname_sim_edge_vtk = [fld_sim, 'simulator_edge.vtk'];
        WriteSimulatorSample(fname_sim_edge_vtk, pt_sample, edge);

        % Output the edge of the extracted simulator in txt format
        fname_sim_edge_txt = [fld_sim, 'simulator_edge.txt'];
        dlmwrite(fname_sim_edge_txt, edge, 'Delimiter', '\t', 'precision', '%d');

        % Output the mapping (Extracted simple mesh to the simulator)
        fname_sim_S2Dmapping = [fld_sim, 'simulator_S2Dmapping.txt'];
        dlmwrite(fname_sim_S2Dmapping, S2Dmapping - 1, 'Delimiter', ',', 'precision', '%d');
        
        % Output the mapping (Extracted simulation mesh to the ML simulator)
        fname_M2Smapping = [fld_sim, 'M2Smapping.txt'];
        fname_mesh_S2Dmapping = [fld_sim, 'mesh_S2Dmapping.txt'];
        mesh_S2Dmapping = dlmread(fname_mesh_S2Dmapping);
        n_mesh = size(mesh_S2Dmapping,2);
        idx_mesh =1:n_mesh;
        LUT_mesh = [];
        LUT_mesh(mesh_S2Dmapping + 1) = idx_mesh;
        M2Smapping = LUT_mesh(S2Dmapping);
