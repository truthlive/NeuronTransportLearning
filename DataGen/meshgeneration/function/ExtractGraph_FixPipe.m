function [num_pipe, num_bif] = ExtractGraph_FixPipe(io_path)
    ...
    for i = 1:n_sect
        ...
        [tmp_XYZ, tmp_D, tmp_tangent, tmp_n_chop] = BsplineSmoothEqualSeg_FixPipe(location(sect_point{i}, :), d(sect_point{i}), mode, n_sample, n_seg_per_chop, len_chop);
        ...
        % Group internal node
        tmp_pipe_idx = zeros(tmp_n_chop * (n_seg_per_chop + 1), 1, 'int64');
        ...
        for j = j_start: j_end
            index = index + 1;
            tmp_pipe_idx(j + n_dup) = index;
            if( mod(j+n_dup, n_seg_per_chop+1) == 0 && j < j_end)
                n_dup = n_dup + 1;
                tmp_pipe_idx(j + n_dup) = index;
            end         
        end

        tmp_pipe_idx = reshape(tmp_pipe_idx,n_seg_per_chop + 1,tmp_n_chop);
        vec_idx_graph = [idx_graph:(idx_graph+tmp_n_chop -1)];
        GraphCnct = [GraphCnct,[vec_idx_graph(2:end)-1; ones(1,tmp_n_chop-1)]];
        idx_graph = idx_graph + tmp_n_chop;
        tmp_pipe_idx = [vec_idx_graph;tmp_pipe_idx];
        AllPipe = [AllPipe;tmp_pipe_idx'];
        ...
        for idx_chop = 1:tmp_n_chop
            ...
            output_pipe_file = [pipe_path, 'pipe_', num2str(num_pipe, '%04d'), '.swc'];
...
%                 if AllBif(row_b, 4) == AllPipe(row_p,2)
%                     tmpXYZ = output_graph_mat(GraphCnct(i,1),3:5) + vec_bif2pipe(1,:);
%                 else
%                     tmpXYZ = output_graph_mat(GraphCnct(i,1),3:5) + vec_bif2pipe(2,:);
%                 end
%             else
%                 tmpXYZ = output_graph_mat(GraphCnct(i,1),3:5) + vec_pipe2pipe;
%             end
%         else
%             tmpXYZ = output_graph_mat(GraphCnct(i,1),3:5) + vec_pipe2bif;
%         end
...
%     end
    ...
    for i = 1:num_graphnode
        ...
        elseif GraphCnct(i,2)==1
            ...
            if GraphCnct(GraphCnct(i,1),2)==2
                row_b = find(AllBif(:,1) == GraphCnct(i,1));
                tmpXYZ = (XYZ2(AllPipe(row_p,2),:) - XYZ2(AllBif(row_b,2),:))/...
                    norm(XYZ2(AllPipe(row_p,2),:) - XYZ2(AllBif(row_b,2),:));
            else
                tmpXYZ = (XYZ2(AllPipe(row_p,end),:) - XYZ2(AllPipe(row_p,2),:))/...
                    norm(XYZ2(AllPipe(row_p,end),:) - XYZ2(AllPipe(row_p,2),:));
            end
            ...
            tmpXYZ = (XYZ2(AllBif(row_b,2),:) - XYZ2(AllPipe(row_p,end),:))/...
                norm(XYZ2(AllBif(row_b,2),:) - XYZ2(AllPipe(row_p,end),:));
            tmpXYZ = tmpXYZ + output_graph_mat(GraphCnct(i,1),3:5);
            output_graph_mat = [output_graph_mat;[i,2,tmpXYZ,0.1,GraphCnct(i,1)]];
