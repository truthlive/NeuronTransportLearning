function [] = GenHexMesh(io_path, pipe_start, bif_start)
    %% User settings
    ...
    sim_path = [io_path, 'MeshComponent//'];
    ...
    extract_p = [[0:m1 - 1]', [0:m1 - 1]'];
    extract_merge_p = [[0:m3 - 1]', [0:m3 - 1]'];

    extract_p(:, 2) = extract_p(:, 2) + 1;
    extract_merge_p(:, 2) = extract_merge_p(:, 2) + 1;

    extract_e = template_e;
    extract_merge_e = template_merge_e;

    extract_merge_e_bot = template_branch_bottom;
    extract_merge_e_left = template_branch_left;
    extract_merge_e_right = template_branch_right;
    ...
    if n_bif ~= 0
        ...
        for index_sec = 1:n_sect
            ...
            if angle_total > pi / 4 && angle_total <= 3 * pi / 4
                ...
                if dot(rotate_axis, n_end) > 0
                    ...
                    for index_ele = 1:m2_extract / 4
                        tmp_end_element_extract(index_ele + m2_extract / 4 * 0, :) = NodeLayer_extract{index_end_pt, 2}(index_ele + m2_extract / 4 * 3, [3 4 1 2]);
                        tmp_end_element_extract(index_ele + m2_extract / 4 * 1, :) = NodeLayer_extract{index_end_pt, 2}(index_ele + m2_extract / 4 * 0, :);
                        tmp_end_element_extract(index_ele + m2_extract / 4 * 2, :) = NodeLayer_extract{index_end_pt, 2}(index_ele + m2_extract / 4 * 1, [3 4 1 2]);
                ...
                elseif dot(rotate_axis, n_end) < 0
                    ...
                    for index_ele = 1:m2_extract / 4
                        ...
                        tmp_end_element_extract(index_ele + m2_extract / 4 * 1, :) = NodeLayer_extract{index_end_pt, 2}(index_ele + m2_extract / 4 * 2, [3 4 1 2]);
                        tmp_end_element_extract(index_ele + m2_extract / 4 * 2, :) = NodeLayer_extract{index_end_pt, 2}(index_ele + m2_extract / 4 * 3, :);
                        tmp_end_element_extract(index_ele + m2_extract / 4 * 3, :) = NodeLayer_extract{index_end_pt, 2}(index_ele + m2_extract / 4 * 0, [3 4 1 2]);
                    end
            ...
            elseif angle_total > 3 * pi / 4 && angle_total <= pi
                ...
                for index_ele = 1:m2_extract / 4
                    tmp_end_element_extract(index_ele + m2_extract / 4 * 0, :) = NodeLayer_extract{index_end_pt, 2}(index_ele + m2_extract / 4 * 2, [3 4 1 2]);
                    tmp_end_element_extract(index_ele + m2_extract / 4 * 1, :) = NodeLayer_extract{index_end_pt, 2}(index_ele + m2_extract / 4 * 3, [3 4 1 2]);
                    tmp_end_element_extract(index_ele + m2_extract / 4 * 2, :) = NodeLayer_extract{index_end_pt, 2}(index_ele + m2_extract / 4 * 0, [3 4 1 2]);
                    tmp_end_element_extract(index_ele + m2_extract / 4 * 3, :) = NodeLayer_extract{index_end_pt, 2}(index_ele + m2_extract / 4 * 1, [3 4 1 2]);
                end
    ...
    for idx_sim = 1:n_pipe_sim
        ...
        [n_pt_sim, ~] = size(pt_sim);
        label_sim = ones(n_pt_sim, 1);
        label_sim = in_point_label * label_sim;
        label_sim(1:m1_extract) = 1;

        for idx_layer = 0:(n_pt_sim / m1_extract - 1)
            label_sim(idx_layer * m1_extract + boundary_point_index, :) = wall_label;
        end

        fld_sim = [sim_path, 'Pipe//', num2str(n_pipe_start + idx_sim, '%04d'), '//'];

        if ~exist(fld_sim, 'dir')
            mkdir(fld_sim)
        end

        fname_sim = [fld_sim, 'controlmesh.vtk'];
        WriteVTK(fname_sim, pt_sim, ele_sim_local, label_sim, []);

        vel_sim = [];
        vel_sim(C2Smapping(:, 2) + 1, :) = AllVelocity(C2Smapping(:, 1), :);

        %         tmp_velocity = zeors(size(pt_sim));
        fname_vel = [fld_sim, 'initial_velocity.txt'];
        dlmwrite(fname_vel, vel_sim, 'Delimiter', ' ', 'precision', '%f');

        % Output the mapping (Extracted simple mesh to the simulator)
        fname_sim_S2Dmapping = [fld_sim, 'mesh_S2Dmapping.txt'];
        dlmwrite(fname_sim_S2Dmapping, S2Dmapping - 1, 'Delimiter', ',', 'precision', '%d');
        
        % write controlmesh info
        fname_tmeshinfo = [fld_sim, 'controlmesh_info.txt'];
        dlmwrite(fname_tmeshinfo, [size(pt_sim,1), size(ele_sim_local,1)], 'Delimiter', ' ', 'precision', '%d');

    end

    % Bif simulator
    n_bif_sim = size(dict_bif, 1);
    n_bif_start = bif_start;
    ...
    for idx_sim = 1:n_bif_sim
        ...
        [n_pt_sim, ~] = size(pt_bif);
        label_sim = ones(n_pt_sim, 1);
        label_sim = in_point_label * label_sim;
        label_sim((m3_extract + 1):(m3_extract + m1_extract)) = 1;
        label_sim(boundary_merge_point_index, :) = wall_label;

        for idx_layer = 0:((n_pt_sim - m3_extract) / m1_extract - 1)
            label_sim(m3_extract + idx_layer * m1_extract + boundary_point_index, :) = wall_label;
        end

        fld_sim = [sim_path, 'Bif//', num2str(n_bif_start + idx_sim, '%04d'), '//'];
        ...
        fname_sim = [fld_sim, 'controlmesh.vtk'];
        WriteVTK(fname_sim, pt_bif, ele_sim_local, label_sim, []);
