...
%% User settings
% clearvars -except idx_geo
% start_trees;
% idx_geo
skeleton_path = '..//..//MLdata//skeleton_pipe//';
% io_path = '..//..//MLdata//test1_OnlyGeofeature//';
% io_path = '..//..//MLdata//test4_PipeNew//';
io_path = '..//..//MLdata//3bifurcation//';

% parameter_file=[io_path,'mesh_parameter.txt'];
% output_file = [pipe_path, 'pipe_', num2str(num_pipe), '.swc'];
% skeleton_input = [skeleton_path, 'pipe_', num2str(idx_geo,'%04d'), '.swc'];
% velocity_output=[io_path,num2str(idx_geo,'%04d'),'//initial_velocityfield.txt'];
% hex_output=[io_path,num2str(idx_geo,'%04d'),'//controlmesh.vtk'];

% Debug 3 bifurcation model
skeleton_input = [io_path, 'geo_3bifurcation.swc'];; 
parameter_file=[io_path,'mesh_parameter.txt'];
velocity_output=[io_path,'initial_velocityfield.txt'];
hex_output=[io_path,'controlmesh.vtk'];
mesh_output = [io_path,'//mesh_extract.vtk'];
graph_output = [io_path,'//graph_extract.vtk'];
...
extract_p_index = load('.//template//template_circle_point90_extract_index.txt');
extract_merge_p_index = load('.//template//template_merge120_points90_extract_index.txt');

extract_edge_index = load('.//template//template_circle_edges_extract_index.txt');
extract_merge_edge_index = load('.//template//template_merge_edges_extract_index.txt');

extract_elements_index = load('.//template//template_circle_elements_extract_index.txt');
extract_merge_elements_index = load('.//template//template_merge_elements_extract_index.txt');

% extract_p_index = [0 1 2 3 7 13 20 27 35 62 96 108 112 119 128 155 189];
% extract_merge_p_index = [0	1	2	3	7	13	20	27	35	62	96	108	112	119	128	155	189	201	205	212	221	248	282];
